...
**Platform.** SwiftUI iPhone-only ([`TARGETED_DEVICE_FAMILY = 1`](../project.yml)), iOS 18.4+, Swift 6 with strict concurrency, `@Observable` throughout. Apple frameworks, the local [`WXYCAPI`](../Packages/WXYCAPI/) and vendored [`WXYCAPIModels`](../Packages/WXYCAPIModels/) SPM packages, and exactly one third-party package: [`sentry-cocoa`](https://github.com/getsentry/sentry-cocoa) for crash/error reporting, linked to the app target only and kept out of both local packages so they stay pure Foundation and host-testable — see [ADR 0007](./adr/0007-ios-anonymous-error-reporting.md).
...
| **[wxyc-shared](https://github.com/WXYC/wxyc-shared)** | OpenAPI source of truth ([`api.yaml`](https://github.com/WXYC/wxyc-shared/blob/main/api.yaml)) and shared TS contracts | Every new BS endpoint gets documented in `api.yaml`. iOS codegens from `api.yaml` since issue #75 — `Packages/WXYCAPIModels` is vendored generator output with a CI drift gate. A minority of DTOs stay hand-authored where the schema doesn't match the real handler; see [`CLAUDE.md`](../CLAUDE.md)'s "Code Generation" |
...
| Wanting per-decision detail | [`adr/`](./adr/) — local-canonical sources for 4 of the 9 cross-repo ADRs: [0001 entity_id](./adr/0001-entity-id-canonical-artist-identifier.md), [0002 QR device auth](./adr/0002-qr-device-authorization-shared-computer-signin.md), [0003 per-album play stats](./adr/0003-per-album-play-stats.md), [0004 Search Plays + filter builder](./adr/0004-search-plays-flowsheet-builder.md) (the other five live only in `cross-repo-adrs.md`), plus three local-only decisions taken after the v2 design and outside the nine: [0005 iOS Spotlight on-device catalog clone](./adr/0005-ios-spotlight-on-device-catalog-clone.md), a 14th surface ([#19](https://github.com/WXYC/wxyc-dj-ios/issues/19)), [0006 iOS leads sign-in with a mailed one-time code](./adr/0006-ios-otp-primary-sign-in.md) ([#100](https://github.com/WXYC/wxyc-dj-ios/issues/100)), and [0007 iOS reports errors with a hard anonymity floor](./adr/0007-ios-anonymous-error-reporting.md) ([#106](https://github.com/WXYC/wxyc-dj-ios/issues/106)) |
