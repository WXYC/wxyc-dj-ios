...
- ~~**`'N'` is not legacy** — it is a current member of the server rotation enum (`apps/backend/services/library.service.ts:240`; BS `apps/backend/app.yaml`).~~ **Superseded (BS#2173):** `'N'` was never a rotation bin. It was a sentinel two Backend-Service writers used for a release with no usable bin, and it is gone from `freq_enum`. Tubafrenzy's real "New" is flowsheet entry-type code 5 ("new vinyl, not yet in rotation"), which is explicitly not a bin and means the opposite of a rotation weight.
