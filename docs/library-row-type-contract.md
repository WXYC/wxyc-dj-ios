...
- **~~This app keeps hand-rolled DTOs for now~~ — SUPERSEDED by issue #75 (2026-08).** The repo now vendors `Packages/WXYCAPIModels`, generated from `api.yaml` via `wxyc-shared`'s `openapi-generator-cli` pipeline, with a CI drift gate. A solution relying on iOS codegen is therefore no longer a larger commitment than this repo has made — but note the boundary that motivates this whole document: the generator proves the Swift mirror matches `api.yaml`, **not** that `api.yaml` matches the handler. Where those disagree the DTO stays hand-authored; see `CLAUDE.md`'s "Code Generation" table. Still true: no third-party runtime packages, explicit `CodingKeys`, `convertFromSnakeCase` deliberately off.
