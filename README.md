...
WXYCDJ/                          # App target sources
  ...
  Bin/                           # BinView + VM
  Telemetry/                     # error reporting (#106) + product analytics (#108): ErrorReporter
                                 #   and Analytics seams, privacy scrub + allowlist filter, build
                                 #   environment, Sentry/PostHog bootstrap (the two SDK imports)
...
Issue [#108](https://github.com/WXYC/wxyc-dj-ios/issues/108) adds anonymous PostHog product analytics — search reliability, sign-in method mix, catalog-refresh health, and the other wave-1 events documented in `CLAUDE.md`'s "Telemetry" section — under the same no-PII contract [ADR 0007](docs/adr/0007-ios-anonymous-error-reporting.md) sets for Sentry. `PostHogSDK.identify(...)` is never called and `personProfiles` is `.never`, so every event is anonymous, not merely pseudonymous-by-default.

**Events go to dj-site's PostHog project (444958), not a project of this app's own.** dj-site is the web equivalent of this app — the same DJs doing the same jobs — so sharing one project turns "phone or desktop for library search?" into a breakdown on `$lib` instead of an unanswerable question. Two things follow: event names must stay collision-free across both repos (a reused name puts two schemas under one event), and privacy contract item 3 — no IP retention — is the project-wide `anonymize_ips` server setting rather than anything the app can set, so enabling it also drops dj-site's geo breakdown going forward. Both are covered in `CLAUDE.md`'s Telemetry section; the project token is hardcoded in `WXYCDJ/Telemetry/TelemetryBootstrap.swift` for the same reason the Sentry DSN is.
...
A signed-in DJ (any role but `member` — the JWT role `digital_archive` denies) can stream a bound album straight from the auto DJ's library: an album detail screen carrying the digital-audio badge shows a "Play" section listing that album's manifest tracks, and a mini-player at the bottom of the tab bar keeps the transport reachable from Search or Bin. Playback is in-memory streaming only — no downloads, nothing cached beyond ordinary `AVPlayer` buffering — behind a signed, time-limited manifest and a server-side kill switch (`DIGITAL_ARCHIVE_STREAMING_ENABLED`). It continues in the background and publishes the playing track to the Lock Screen and Control Center, where play/pause/next/previous work. It pauses for an incoming call or a route disconnect (unplugged headphones) and resumes afterward — but only if you hadn't paused it yourself in the meantime. Signing out stops it: a presigned URL outlives the session, so the app hands the audio back rather than leaving a signed-out device streaming with no transport left on screen.
...
Telemetry is **no longer deferred** — Sentry error reporting shipped in [#106](https://github.com/WXYC/wxyc-dj-ios/issues/106) (dSYM upload for symbolicated Release traces in [#107](https://github.com/WXYC/wxyc-dj-ios/issues/107)), and PostHog product analytics shipped in [#108](https://github.com/WXYC/wxyc-dj-ios/issues/108) under the same no-PII contract, reporting into dj-site's shared project — see "Product Analytics (PostHog)" above. See [`docs/adr/0007-ios-anonymous-error-reporting.md`](docs/adr/0007-ios-anonymous-error-reporting.md) for what is and isn't collected by either SDK.
