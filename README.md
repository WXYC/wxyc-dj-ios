...
**A build with an empty/placeholder PostHog project token is fully functional and analytics-free.** `TelemetryBootstrap.startAnalytics()` refuses to start the SDK while `postHogAPIKey` still equals the checked-in placeholder (`"phc_REPLACE_ME_WXYC_DJ_PROJECT_TOKEN"`) — the WXYC org's "WXYC DJ" PostHog project doesn't exist yet as of this writing (creating it requires first deleting a dormant project to clear the org's free-tier project limit; see issue #108 for the full sequencing). Once that project exists, swap the placeholder in `WXYCDJ/Telemetry/TelemetryBootstrap.swift` for the real project token — no other code changes needed.
