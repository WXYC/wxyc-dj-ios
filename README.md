...
- **Background (best-effort, no guaranteed scheduling):** a split-task design driven by `BackgroundTasks`:
  ...
  - `org.wxyc.dj.catalog.reindex` — a `BGProcessingTask` (reindex leg, `requiresExternalPower`) registered by `AppDelegate` (SwiftUI has no `.processing` matcher). Does the multi-MB download + ~50k-row decode + Spotlight reindex while charging. Idempotent: it re-derives "is there still a `200`?" from its own conditional GET, so a foreground refresh that already ran makes it a cheap `304`.
